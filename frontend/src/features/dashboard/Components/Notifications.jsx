...
const Notifications = ({ notifications, isOpen}) => {
  ...
  const markAsRead = async (id) => {
    setLoading(true)
    try{
      const { data } = await api.post(`/users/notifications/${id}`)
      return data
    }catch(err){
      setErrorMessage(err.response?.data?.message || 'Error reading notification')
    }finally{
      setLoading(false)
    }
  }
                ...
                <div className="flex-1">
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <span
                      ...
                      `}
                    >
                      {notification.read === false ? "New" : "Read"}
                    </span>
                  </div>
